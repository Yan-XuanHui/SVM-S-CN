...
if nargin< 12            %nargin是matlab判断输入变量个数的函数
    alphainit=[];
end;
...
if nargin < 13
    ps  =  zeros(n,n);		
    %ps=svmkernel(x,kernel,kerneloption);    %ps是核矩阵
    %计算符号数据核矩阵，对于多类只要算一次存储起来用
    ps = svmkernel_categorical(x, kernel, kerneloption, dataSta, dotProduct, my_lambda);   %高斯核函数不需要参数dotProduct点积矩阵
end;
%----------------------------------------------------------------------
